...
def check_intra_linter_bare(ref, guide_linter, compound_linters):
    ...
    return bool(guide_linter and guide_linter in compound_linters and ref in compound_linters[guide_linter])
...
def main():
    ...
    for filepath in guide_files:
        ...
        for ref in refs:
            ...
            if ref in seen:
                ...
                dup_violation_details.append(
                    f"  {os.path.relpath(filepath, os.path.dirname(GUIDES_ROOT))}: duplicate '{ref}'"
                )
            else:
                seen[ref] = True
        ...
        for ref in refs:
            is_valid, reason = check_format_compliance(ref, compound_linters, simple_linters, reverse_lookup)
            if not is_valid:
                format_violations += 1
                format_violation_details.append(
                    f"  {os.path.relpath(filepath, os.path.dirname(GUIDES_ROOT))}: '{ref}' — {reason}"
                )
    ...
    try:
        ...
        integrity_status = (
            "CLEAN" if non_related_changes == 0 else f"WARNING: {non_related_changes} potential non-related changes"
        )
        ...
        try:
            for detail in format_violation_details:
                ...
                if m:
                    ...
                    result = subprocess.run(
                        ["git", "show", f"HEAD~1:{filepath_rel}"], capture_output=True, text=True, cwd=project_root
                    )
    ...
    try:
        ...
        _plan_commits = len([line for line in result.stdout.strip().split("\n") if line])
        ...
        plan02_files = len([line for line in result1.stdout.strip().split("\n") if line])

        print(f"Plan 02 files modified: {plan02_files}")
        print(f"Total references validated: {total_refs}")
        print(f"Guides with populated related tags: {guides_with_related}")
        print(f"Guides with empty related tags: {guides_empty_related}")
        print("Normalization applied: bare → linter/rule format")
    except Exception:
