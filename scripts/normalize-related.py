...
def process_guide(filepath, reverse_lookup, compound_linters, simple_linters):
    ...
    for ref in refs:
        normalized, changed, reason = normalize_ref(ref, guide_linter, reverse_lookup, compound_linters, simple_linters)
...
def main():
    ...
    if unrecognized:
        ...
        for relpath, _ref, reason in unrecognized:
            print(f"  {relpath}: {reason}")

    print()
    print(
        f"Result: {total_normalizations} normalizations + "
        f"{total_duplicates_removed} dedup across {total_modified} files"
    )
