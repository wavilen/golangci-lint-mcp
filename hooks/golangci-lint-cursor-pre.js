#!/usr/bin/env node
'use strict';
// PreToolUse hook for Cursor — modifies golangci-lint Shell commands before execution to ensure JSON output format. Always exits 0.
...
process.stdin.on('end', function () {
  try {
    ...
    if (shared.hasGolangciLintMcp()) {
      ...
      console.log(JSON.stringify({ decision: 'allow', updated_input: { command: interceptCmd } }));
      process.exit(0);
    }
    ...
    if (parts.echoSuffix) {
      ...
      const segs = shared.splitOnUnquotedSemicolons(inner).filter(Boolean);
    ...
    // Cursor output format: { decision: 'allow', updated_input: { command: modified } }
    console.log(JSON.stringify({ decision: 'allow', updated_input: { command: modified } }));
    process.exit(0);
