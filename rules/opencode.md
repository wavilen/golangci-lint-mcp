...
1. **Run and get guidance.** Call `golangci_lint_run` with a package path (e.g., `golangci_lint_run(path="./pkg/auth/...")`). It runs golangci-lint, parses results, and returns fix guidance with Related Context for related issues. The `/golangci-lint` command runs golangci-lint with correct flags and feeds output to MCP tools automatically.
