.PHONY: build install install-skill install-commands install-rules install-hook install-claude-shared install-all test clean npm-pack npm-publish update-golden-config crosscheck crosscheck-clean install-plugin verify-plugin verify-shared sync-deployed
